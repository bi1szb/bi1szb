...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("术中前列腺超声可视化工具 v2")
        ...
        self.cut_point_combo.addItems(POINT_NAMES)
    ...
    def initialize_cutting_plan(self):
        ...
        for idx, name in enumerate(POINT_NAMES):
            ...
            angles = self.normalize_cut_angles(BUTTERFLY_ANGLES if is_butterfly_name(name) else NORMAL_ANGLES)
        ...
        self.selected_cut_name = "ML"
        self.cut_point_combo.setCurrentText("ML")
    ...
    def redraw_cutting_overlay(self):
        ...
        for name in POINT_NAMES:
            if name in self.cut_plan:
                point = self.cut_plan[name]
                display_points[name] = self.sagittal_point_for_radius(point, self.point_primary_radius(point))
        if "VZ" in self.cut_plan and "END" in self.cut_plan:
            vz = self.cut_plan["VZ"]
            end = self.cut_plan["END"]
            vz_r, vz_l = self.butterfly_side_points(vz)
            end_r, end_l = self.butterfly_side_points(end)
            pairs = [(vz_r, end_r), (vz_l, end_l)]
            radii = [
                (self.point_radii(vz)[0] + self.point_radii(end)[0]) / 2.0,
                (self.point_radii(vz)[1] + self.point_radii(end)[1]) / 2.0,
            ]
        ...
        line_pairs = []
        for left_name, right_name in [("ML", "BN"), ("BN", "MP")]:
            if left_name in display_points and right_name in display_points:
                line_pairs.append((display_points[left_name], display_points[right_name]))
        if "MP" in display_points and "VZ" in self.cut_plan:
            line_pairs.append((display_points["MP"], display_points["VZ"]))
        if "VZ" in self.cut_plan and "END" in self.cut_plan:
            vz_r, vz_l = self.butterfly_side_points(self.cut_plan["VZ"])
            end_r, end_l = self.butterfly_side_points(self.cut_plan["END"])
            line_pairs.extend([(vz_r, end_r), (vz_l, end_l)])
    ...
    def end_sagittal_cut_drag(self):
        self._dragging_sagittal_cut = False
        self.redraw_cutting_overlay()
        if self.selected_cut_name in POINT_NAMES and self.cut_point_combo.currentText() != self.selected_cut_name:
    ...
    def is_butterfly_index(self, index: int):
        if "VZ" not in self.cut_plan or "END" not in self.cut_plan:
            return False
        a = self.cut_plan["VZ"].transverse_index
        b = self.cut_plan["END"].transverse_index
        return min(a, b) <= index <= max(a, b)
    ...
    def cut_display_index_range(self):
        if "ML" not in self.cut_plan or "END" not in self.cut_plan:
            return None
        start = self.cut_plan["ML"].transverse_index
        end = self.cut_plan["END"].transverse_index
    ...
    def interpolated_cut_region(self, index: int):
        if not self.cut_plan:
            return None
        ordered = sorted(self.cut_plan.values(), key=lambda p: p.transverse_index)
    ...
    def sagittal_cut_point_moved(self, name, pos):
        ...
        old_index = point.transverse_index
        ...
        point.sagittal_point = QPointF(pos.x(), baseline + self.point_primary_radius(point))
        self.cut_slice_overrides.pop(old_index, None)
        self.cut_slice_overrides.pop(point.transverse_index, None)
    ...
    def apply_cut_panel_values(self):
        ...
        x = self.point_at_zero_distance(point.transverse_index * max(float(self.tick_pixel_step), 1.0)).x()
        baseline = self.baseline_y_at_x(x)
        point.sagittal_point = QPointF(x, baseline + self.point_primary_radius(point))
        self.cut_slice_overrides.pop(old_index, None)
        self.cut_slice_overrides.pop(point.transverse_index, None)
        self.redraw_cutting_overlay()
        self.update_cut_panel()
    ...
    def planning_json_path(self):
        if not self.patient_dirs or self.patient_index < 0:
            return None
        return self.patient_dirs[self.patient_index] / "planning_line_iter2.json"
    ...
    def load_planning_json(self):
        ...
        try:
            ...
            self.statusBar().showMessage(f"读取 planning_line_iter2.json 失败: {exc}", 8000)

    def cutting_json_path(self):
        if not self.patient_dirs or self.patient_index < 0:
            return None
        return self.patient_dirs[self.patient_index] / "cutting_plan_iter2.json"
    ...
    def load_cutting_json(self):
        ...
        try:
            ...
            for name in POINT_NAMES:
                ...
                raw_index = int(sagittal.get("transverse_index", 1 if version >= 3 else 0))
                ...
                angles = self.normalize_cut_angles(transverse.get("angles", BUTTERFLY_ANGLES if is_butterfly_name(name) else NORMAL_ANGLES))
                ...
                point = CuttingPlanPoint(
                    name=name,
                    sagittal_point=list_to_qpoint(sagittal.get("point", [0, 0])),
                    transverse_index=transverse_index,
                    center=list_to_qpoint(transverse.get("center", [0, 0])),
                    radius=min(radii) if len(radii) == 2 else radii[0],
                    angles=angles,
                    radii=radii,
                )
            ...
            for raw_index, raw in data.get("slice_overrides", {}).items():
                try:
                    index = int(raw_index)
                except (TypeError, ValueError):
                    continue
                if index < 0 or (self.transverse_paths and index >= len(self.transverse_paths)):
                    continue
                _, key_point = self.key_point_at_index(index)
                if key_point is None:
                    continue
                key_point.center = list_to_qpoint(raw.get("center", qpoint_to_list(key_point.center)))
                key_point.angles = self.normalize_cut_angles(raw.get("angles", key_point.angles))
                self.set_point_radii(key_point, raw.get("r", raw.get("radius", self.point_radii(key_point))))
                baseline = self.baseline_y_at_x(key_point.sagittal_point.x())
                key_point.sagittal_point = QPointF(key_point.sagittal_point.x(), baseline + self.point_primary_radius(key_point))
            self.scale_confirmed = bool(data.get("scale_confirmed", bool(self.cut_plan)))
            self.selected_cut_name = data.get("selected", "ML")
            if self.selected_cut_name in POINT_NAMES:
                self.cut_point_combo.setCurrentText(self.selected_cut_name)
            self.redraw_cutting_overlay()
            self.update_cut_panel()
            self.update_transverse_cut_overlay()
        except Exception as exc:
            self.statusBar().showMessage(f"读取 cutting_plan_iter2.json 失败: {exc}", 8000)
